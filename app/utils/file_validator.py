...
async def validate_and_read_file(file: UploadFile) -> tuple[bytes, str, str]:
    ...
    if len(parts) != 2 or not parts[1]:
        raise HTTPException(
            status_code=400, detail="Filename must have a valid extension"
        )

    ext = parts[1].lower()
    if ext not in settings.ALLOWED_EXTENSIONS:
        raise HTTPException(
            status_code=400, detail=f"Extension '.{ext}' is not permitted"
        )
    ...
    if detected_mime not in allowed_mimes:
        raise HTTPException(
            status_code=400,
            detail=f"File content type '{detected_mime}' does not match extension '.{ext}'",
        )
